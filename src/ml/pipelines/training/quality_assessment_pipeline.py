"""
Quality Assessment Training Pipeline

This module implements the training pipeline for quality assessment models in PBF-LB/M processes.
It handles data ingestion, preprocessing, feature engineering, model training, and evaluation
for quality score prediction, dimensional accuracy, surface finish, and mechanical properties.
"""
...
class QualityAssessmentTrainingPipeline(BaseTrainingPipeline):
    """
    Training pipeline for quality assessment models.
    
    This pipeline handles the complete training workflow for quality assessment models including:
    ...
    def __init__(self, config_manager=None):
        """
        Initialize the quality assessment training pipeline.
        
        Args:
            config_manager: Configuration manager instance
        """
        super().__init__("quality_assessment_pipeline", config_manager)
        ...
        logger.info("Initialized QualityAssessmentTrainingPipeline")
    
    def _execute_model_training_stage(self, config: Dict[str, Any]) -> Dict[str, Any]:
        """
        Execute model training stage for quality assessment models.
        ...
        """
        logger.info("Executing quality assessment model training stage")
        ...
        for model_name, model in self.models.items():
            ...
            try:
                ...
                if model_name == 'quality_score_predictor':
                    # Train quality score prediction model
                    trained_model, history = self._train_quality_score_predictor(
                        model, X_train, y_train, X_val, y_val, training_params
                    )
                elif model_name == 'dimensional_accuracy_predictor':
                    # Train dimensional accuracy prediction model
                    trained_model, history = self._train_dimensional_accuracy_predictor(
                        model, X_train, y_train, X_val, y_val, training_params
                    )
                elif model_name == 'surface_finish_predictor':
                    # Train surface finish prediction model
                    trained_model, history = self._train_surface_finish_predictor(
                        model, X_train, y_train, X_val, y_val, training_params
                    )
                elif model_name == 'mechanical_property_predictor':
                    # Train mechanical property prediction model
                    trained_model, history = self._train_mechanical_property_predictor(
                        model, X_train, y_train, X_val, y_val, training_params
                    )
    ...
    def _prepare_training_data(self, data: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        """
        Prepare training data for quality assessment models.
        ...
        target_columns = ['quality_score', 'dimensional_accuracy', 'surface_roughness', 'tensile_strength']
        feature_columns = [col for col in data.columns if col not in target_columns]
        
        X = data[feature_columns].values
        y = data[target_columns[0]].values  # Primary target: quality_score
        
        return X, y
    
    def _train_quality_score_predictor(self, model, X_train, y_train, X_val, y_val, training_params):
        """
        Train the quality score predictor model.
        ...
        """
        # Use ensemble methods for quality score prediction
        ensemble_models = {
    ...
    def _train_dimensional_accuracy_predictor(self, model, X_train, y_train, X_val, y_val, training_params):
        """
        Train the dimensional accuracy predictor model.
    ...
    def _train_surface_finish_predictor(self, model, X_train, y_train, X_val, y_val, training_params):
        """
        Train the surface finish predictor model.
    ...
    def _train_mechanical_property_predictor(self, model, X_train, y_train, X_val, y_val, training_params):
        """
        Train the mechanical property predictor model.
    ...
    def _execute_feature_engineering_stage(self, config: Dict[str, Any]) -> Dict[str, Any]:
        """
        Execute feature engineering stage with quality assessment specific features.
        ...
        Returns:
            Stage execution results
        """
        logger.info("Executing quality assessment feature engineering stage")
        ...
        # Apply quality-specific feature engineering
        ...
        if any(col in preprocessed_data.columns for col in ['defect_type', 'severity', 'area']):
            defect_data = preprocessed_data[['defect_type', 'severity', 'area']].fillna(0)
            defect_features = self.feature_engineers['defect_image'].extract_all_features(defect_data)
            engineered_features = pd.concat([engineered_features, defect_features], axis=1)
        ...
        if any(col in preprocessed_data.columns for col in ['laser_power', 'scan_speed', 'layer_height']):
            process_data = preprocessed_data[['laser_power', 'scan_speed', 'layer_height']].fillna(0)
            process_features = self.feature_engineers['laser_parameter'].extract_all_features(process_data)
            engineered_features = pd.concat([engineered_features, process_features], axis=1)
        ...
        return {
            ...
            'quality_features_applied': ['ct_scan', 'powder_bed', 'defect_image', 'surface_texture'],
            'process_features_applied': ['laser_parameter', 'material']
        }
    
    def _apply_temporal_features(self, data: pd.DataFrame, config: Dict[str, Any]) -> pd.DataFrame:
        """
        Apply temporal feature transformations for quality assessment.
    ...
    def _apply_statistical_features(self, data: pd.DataFrame, config: Dict[str, Any]) -> pd.DataFrame:
        """
        Apply statistical feature transformations for quality assessment.
    ...
    def _apply_frequency_features(self, data: pd.DataFrame, config: Dict[str, Any]) -> pd.DataFrame:
        """
        Apply frequency feature transformations for quality assessment.
    ...
    def _apply_cross_features(self, data: pd.DataFrame, config: Dict[str, Any]) -> pd.DataFrame:
        """
        Apply cross feature transformations for quality assessment.
        ...
        """
        interactions = config.get('interactions', ['density*defect_count', 'roughness*gloss'])
