"""
Process Optimization Training Pipeline

This module implements the training pipeline for process optimization models in PBF-LB/M processes.
It handles data ingestion, preprocessing, feature engineering, model training, and evaluation
for laser parameter prediction, build strategy optimization, material tuning, and multi-objective optimization.
"""
...
from ...features.process_features import LaserParameterFeatures, BuildParameterFeatures, MaterialFeatures, EnvironmentalFeatures
from ...features.temporal_features import TimeSeriesFeatures, LagFeatures, RollingFeatures, FrequencyFeatures
from ...utils.evaluation import RegressionMetrics, CustomMetrics

logger = logging.getLogger(__name__)


class ProcessOptimizationTrainingPipeline(BaseTrainingPipeline):
    """
    Training pipeline for process optimization models.
    
    This pipeline handles the complete training workflow for process optimization models including:
    ...
    def __init__(self, config_manager=None):
        """
        Initialize the process optimization training pipeline.
        
        Args:
            config_manager: Configuration manager instance
        """
        super().__init__("process_optimization_pipeline", config_manager)
        ...
        self.feature_engineers = {
            ...
            'environmental': EnvironmentalFeatures(self.config_manager),
        ...
        self.models = {
            ...
            'material_tuning': MaterialTuningModels(self.config_manager),
        ...
        logger.info("Initialized ProcessOptimizationTrainingPipeline")
    
    def _execute_model_training_stage(self, config: Dict[str, Any]) -> Dict[str, Any]:
        """
        Execute model training stage for process optimization models.
        ...
        """
        logger.info("Executing process optimization model training stage")
        ...
        for model_name, model in self.models.items():
            ...
            try:
                ...
                if model_name == 'laser_parameter_predictor':
                    # Train laser parameter prediction model
                    trained_model, history = self._train_laser_parameter_predictor(
                        model, X_train, y_train, X_val, y_val, training_params
                    )
                elif model_name == 'build_strategy_optimizer':
                    # Train build strategy optimization model
                    trained_model, history = self._train_build_strategy_optimizer(
                        model, X_train, y_train, X_val, y_val, training_params
                    )
                elif model_name == 'material_tuning':
                    # Train material tuning model
                    trained_model, history = self._train_material_tuning(
                        model, X_train, y_train, X_val, y_val, training_params
                    )
                elif model_name == 'multi_objective_optimizer':
                    # Train multi-objective optimization model
                    trained_model, history = self._train_multi_objective_optimizer(
                        model, X_train, y_train, X_val, y_val, training_params
                    )
    ...
    def _prepare_training_data(self, data: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        """
        Prepare training data for process optimization models.
        ...
        target_columns = ['laser_power', 'scan_speed', 'hatch_spacing', 'layer_height', 'quality_score']
        feature_columns = [col for col in data.columns if col not in target_columns]
        
        X = data[feature_columns].values
        y = data[target_columns[0]].values  # Primary target: laser_power
        
        return X, y
    
    def _train_laser_parameter_predictor(self, model, X_train, y_train, X_val, y_val, training_params):
        """
        Train the laser parameter predictor model.
    ...
    def _train_build_strategy_optimizer(self, model, X_train, y_train, X_val, y_val, training_params):
        """
        Train the build strategy optimizer model.
        ...
        Returns:
            Tuple of (trained_model, training_history)
        """
        # Use ensemble methods for build strategy optimization
        ensemble_models = {
    ...
    def _train_material_tuning(self, model, X_train, y_train, X_val, y_val, training_params):
        """
        Train the material tuning model.
    ...
    def _train_multi_objective_optimizer(self, model, X_train, y_train, X_val, y_val, training_params):
        """
        ...
        Returns:
            Tuple of (trained_model, training_history)
        """
        # Multi-objective optimization typically uses specialized algorithms
        # For now, implement a simple multi-output regression model
    ...
    def _execute_feature_engineering_stage(self, config: Dict[str, Any]) -> Dict[str, Any]:
        """
        Execute feature engineering stage with process optimization specific features.
        ...
        Returns:
            Stage execution results
        """
        logger.info("Executing process optimization feature engineering stage")
        ...
        # Apply process-specific feature engineering
        engineered_features = preprocessed_data.copy()
        
        # Laser parameter features
        if any(col in preprocessed_data.columns for col in ['laser_power', 'scan_speed', 'spot_size']):
            laser_data = preprocessed_data[['laser_power', 'scan_speed', 'spot_size']].fillna(0)
        ...
        if any(col in preprocessed_data.columns for col in ['build_orientation', 'support_count', 'layer_height']):
            build_data = preprocessed_data[['build_orientation', 'support_count', 'layer_height']].fillna(0)
        ...
        if any(col in preprocessed_data.columns for col in ['temperature', 'humidity', 'pressure']):
            env_data = preprocessed_data[['temperature', 'humidity', 'pressure']].fillna(0)
            env_features = self.feature_engineers['environmental'].extract_all_features(env_data)
            engineered_features = pd.concat([engineered_features, env_features], axis=1)
        ...
        return {
            ...
            'process_features_applied': ['laser_parameter', 'build_parameter', 'material', 'environmental'],
    ...
    def _apply_temporal_features(self, data: pd.DataFrame, config: Dict[str, Any]) -> pd.DataFrame:
        """
        Apply temporal feature transformations for process optimization.
    ...
    def _apply_statistical_features(self, data: pd.DataFrame, config: Dict[str, Any]) -> pd.DataFrame:
        """
        Apply statistical feature transformations for process optimization.
    ...
    def _apply_frequency_features(self, data: pd.DataFrame, config: Dict[str, Any]) -> pd.DataFrame:
        """
        Apply frequency feature transformations for process optimization.
    ...
    def _apply_cross_features(self, data: pd.DataFrame, config: Dict[str, Any]) -> pd.DataFrame:
        """
        Apply cross feature transformations for process optimization.
        ...
        """
        interactions = config.get('interactions', ['laser_power*scan_speed', 'layer_height*hatch_spacing'])
