"""
Build Strategy Optimizer Model

This module implements a model for optimizing build strategies including
layer thickness, scan patterns, and support structures for PBF-LB/M processes.
"""
...
class BuildStrategyOptimizer(BaseModel):
    """
    Model for optimizing build strategies for PBF-LB/M processes.
    
    This model optimizes:
    - Layer thickness
    - Scan patterns (hatch, contour, infill)
    ...
    def __init__(self, config_manager=None):
        """
        Initialize the build strategy optimizer.
        
        Args:
            config_manager: Configuration manager instance
        """
        super().__init__('build_strategy_optimizer', config_manager)
        self.model_type = self.model_info.get('algorithm', 'neural_network')
        
        logger.info(f"Initialized BuildStrategyOptimizer with algorithm: {self.model_type}")
    ...
    def _build_neural_network(self) -> tf.keras.Model:
        ...
        # Output layer (multiple strategy parameters)
        output_dim = self.architecture.get('output_shape', [5])[0]  # Default 5 parameters
    ...
    def train(self, X_train: np.ndarray, y_train: np.ndarray, 
        ...
        Args:
            ...
            y_train: Training targets (build strategy parameters)
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        Returns:
            Predictions array with build strategy parameters
        """
        try:
    ...
    def _constrain_predictions(self, predictions: np.ndarray) -> np.ndarray:
        ...
        for feature in features:
            if isinstance(feature, dict):
                ...
                ranges[name] = feature.get('range', [0, 1])
        ...
        for i in range(predictions.shape[1]):
            param_name = f'strategy_param_{i}'
    ...
    def optimize_build_strategy(self, part_geometry: Dict[str, float], 
                               material_properties: Dict[str, float],
                               quality_requirements: Dict[str, float]) -> Dict[str, float]:
        """
        Optimize build strategy for specific part and requirements.
        ...
        Returns:
            Dictionary with optimized build strategy parameters
        """
        try:
            # Create feature vector from inputs
            features = self._create_feature_vector(part_geometry, material_properties, quality_requirements)
            
            # Make prediction
            prediction = self.predict(features.reshape(1, -1))[0]
            
            # Map predictions to strategy parameters
            strategy_params = {
                'layer_thickness': float(prediction[0]),
                'scan_pattern': float(prediction[1]),
                'support_density': float(prediction[2]),
                'build_orientation': float(prediction[3]),
                'scan_speed_profile': float(prediction[4])
            }
            
            return strategy_params
            
        except Exception as e:
            logger.error(f"Failed to optimize build strategy: {e}")
            raise
    ...
    def _create_feature_vector(self, part_geometry: Dict[str, float], 
        ...
        all_inputs = {**part_geometry, **material_properties, **quality_requirements}
